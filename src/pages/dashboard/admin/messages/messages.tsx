...
const Messages = () => {
    ...
    const { addToast } = useToast();
    const [messages, setMessages] = useState<any>([]);
    const [loadingButtonState, setLoadingButtonState] = useState<{ [key: number]: string | null }>({});
    ...
    const handleUpdate = (message: any, status: string, index: number) => {
        setLoadingButtonState(prevState => ({ ...prevState, [index]: status }));
        const updateInfo = {
            status,
            subject: message.subject,
            description: message.description,
                        ...
                        <div key={index} className="bg-gray-700 p-4 rounded shadow-md text-white relative">
                            ...
                            <div className="mt-4 space-x-2">
                                <button 
                                    className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded" 
                                    onClick={() => handleUpdate(message, 'approved', index)}
                                    disabled={loadingButtonState[index] !== null}
                                >
                                    {loadingButtonState[index] === 'approved' ? <span className="loading loading-spinner loading-xs"></span> : 'Confirm'}
                                </button>
                                <button 
                                    className="bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded" 
                                    onClick={() => handleUpdate(message, 'rejected', index)}
                                    disabled={loadingButtonState[index] !== null}
                                >
                                    {loadingButtonState[index] === 'rejected' ? <span className="loading loading-spinner loading-xs"></span> : 'Dismiss'}
                                </button>
                            </div>
                        </div>
                    ))}
